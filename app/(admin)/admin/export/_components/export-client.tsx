...
import {
  Download, FileText, Table2, Loader2, Search, Filter, Calendar, ChevronLeft, ChevronRight,
} from "lucide-react";
...
const STATUS_LABELS: Record<string, string> = {
  PENDING: "Menunggu Verifikasi", VERIFIED: "Terverifikasi", CLAIMED: "Selesai (Diklaim)",
  REJECTED: "Ditolak", EXPIRED: "Kedaluwarsa", AWAITING_PICKUP: "Menunggu Diambil",
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
          ...
          <div className="flex flex-col gap-1">
            ...
            <select value={status} onChange={(e) => setStatus(e.target.value)}
              ...
              <option value="AWAITING_PICKUP">Menunggu Diambil</option>
              <option value="CLAIMED">Selesai (Diklaim)</option>
              <option value="REJECTED">Ditolak</option>
              <option value="EXPIRED">Kedaluwarsa</option>
              <option value="RESOLVED">Selesai (Mandiri)</option>
            </select>
          </div>
        </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-slate-50/50 border-b border-slate-100">
                    ...
                    <th className="text-left py-2.5 px-4 font-semibold text-slate-500 text-xs uppercase">Tgl Kejadian</th>
                    <th className="text-left py-2.5 px-4 font-semibold text-slate-500 text-xs uppercase">Tgl Laporan</th>
                  </tr>
                </thead>
                <tbody>
                  {reports.slice((page - 1) * PAGE_SIZE, page * PAGE_SIZE).map((r, i) => (
                    <tr key={r.id} className="border-b border-slate-50 last:border-b-0 hover:bg-orange-50/20">
                      ...
                      <td className="py-2.5 px-4 text-slate-400 whitespace-nowrap">{r.createdAt}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
                ...
                <div key={r.id} className="flex items-center gap-3 p-4">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 text-xs text-slate-400">
                      ...
                      <span>·</span>
                      <span>Kejadian: {r.date}</span>
                      <span>·</span>
                      <span>{STATUS_LABELS[r.status] || r.status}</span>
                    </div>
                    <div className="text-[10px] text-slate-300 mt-0.5">Laporan: {r.createdAt}</div>
                  </div>
                </div>
              ))}
            </div>
            ...
            {Math.ceil(reports.length / PAGE_SIZE) > 1 && (
              <div className="flex items-center justify-center gap-2 py-3 border-t border-slate-100">
                ...
                <span className="text-sm text-slate-600 font-medium">
                  {page} / {Math.ceil(reports.length / PAGE_SIZE)}
                </span>
                <button
                  onClick={() => setPage(Math.min(Math.ceil(reports.length / PAGE_SIZE), page + 1))}
